...
function ssdpObject:parseResponse(data)
  ...
  if self.searchTarget and not (headers.ST and headers.ST == self.searchTarget) then
    return
  end
    ...
    if headers.LOCATION and headers.USN then
      ...
      if usnUUID then
        ...
        self.locations[location] =
          SetTimer(self.timerPrefix .. "Location_" .. location, interval * ONE_SECOND * 1.005, _timer)
...
function ssdpObject:parseXML(strError, responseCode, tHeaders, data, context, url)
  ...
  if responseCode == 200 then
    ...
    local friendlyName =
      XMLDecode(string.match(data, "<" .. self.friendlyNameTag .. ">(.-)</" .. self.friendlyNameTag .. ">"))
