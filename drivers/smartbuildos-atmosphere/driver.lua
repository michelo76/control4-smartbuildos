...
local function discoverPoints(reason)
	...
	nws.points(gLocation.lat, gLocation.lon, function(props, err)
		...
		UpdateProperty(
			"Resolved Location",
			string.format("%s (%.4f, %.4f)", gLocation.label, gLocation.lat, gLocation.lon)
		)
...
local function publishProperties(snapshot)
	...
	UpdateProperty(
		"Data Freshness",
		age ~= nil and string.format("obs %dm ago%s", math.floor(age / 60), snapshot.dataStale and " - STALE" or "")
			or "-"
	)
