...
local function requireTier(needed, label)
	if remoteControlRank() < needed then
		error(
			string.format("%s needs a higher Remote Control tier than this home is set to (Composer property)", label)
		)
	end
end
...
local function postDriverEvent(sku, severity, category, code, message)
	...
	pcall(function()
		...
		}, authHeaders(), { timeout = REQUEST_TIMEOUT }):next(function() end, function() end)
	end)
end
...
local function redeemPairingCode(code)
	...
	}, { ["Content-Type"] = "application/json" }, { timeout = REQUEST_TIMEOUT }):next(function(response)
...
local function requestAccountCode(accountNumber)
	...
	}, { ["Content-Type"] = "application/json" }, { timeout = REQUEST_TIMEOUT }):next(function()
		UpdateProperty(
			"Connection Status",
			"If that account matches, a code was emailed. Enter it in Verification Code."
		)
	end, function(err)
		local code_ = err and err.code
...
function EC.SBOS_REGISTER_DRIVER(tParams)
	...
	if
		isPaired()
		and (cache == nil or cache.assertions[sku] == nil)
		and os.time() - gEntitlements.lastAttempt > 300
	then
...
local function forwardDeviceRoster(source, roster)
	...
	pcall(function()
		http:post(url, { driver_sku = sku, devices = devices }, authHeaders(), { timeout = REQUEST_TIMEOUT })
			:next(function()
				log:debug("Forwarded %d device(s) to Driver Cloud", #devices)
			end, function(err)
				log:debug(
					"Device roster forward failed (non-fatal): %s",
					tostring(err and (err.error or err.code) or err)
				)
			end)
	end)
end
